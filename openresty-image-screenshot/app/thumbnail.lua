...
if not table.contains(c.cfg, uri) then
	...
	if not img_exist then
		if not c.enabled_default_img then
			...
			img_exist=io.open(ngx_img_root ..  c.default_img_uri, ngx.ERR)
			if img_exist then
				lua_log(img_original_uri .. ' is not exist! crop image with default image', ngx.ERR)
    ...
    lua_log('value[ngx_thumbnail_root]==='..img_thumbnail_path, ngx.ERR)
	lua_log("gm_command.."..gm_command, ngx.ERR)
    if (gm_command) then
